...
export default function Wishlist() {
  ...
  if (!wishlist || wishlist.data.length === 0) {
    return (
      <img src="/no_wishlist.png" alt="Your wishlist is empty." className='mx-auto w-3/5 max-h-95 mt-6' />
    );
  }
    ...
    <div className="mx-auto px-6 py-2">
      <div className='flex items-center justify-center gap-3'>
        ...
        <span className='flex items-center gap-1 bg-destructive/10 py-1 px-3 rounded-md text-destructive font-semibold'>
          <Heart size={19} className='fill-destructive'/> {wishlistCount}
        </span>
      </div>
        ...
        {wishlist?.data.map((entry) => (
          ...
          >
            ...
            <Button
                ...
                // className="rounded-full bg-white/90 px-2 py-1 text-xs text-red-500 shadow hover:bg-white"
                >
                Remove
            </Button>
          </div>
